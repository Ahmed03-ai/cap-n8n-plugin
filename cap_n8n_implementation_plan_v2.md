...
### US 1.1 — Trigger Workflow on Entity Creation

**As a** CAP developer,
**I want to** annotate my CDS entities with `@n8n.workflow.start`
**So that** an n8n workflow is automatically triggered via the n8n API when an entity is created.
...
- [ ] Unit tests cover the happy path (workflow triggered) and error path (n8n API unreachable).

---

### US 1.2 — Trigger Workflow on Entity Update and Delete

**As a** CAP developer,
**I want to** annotate my CDS entities with `@n8n.workflow.start`
**So that** an n8n workflow is automatically triggered via the n8n API when an entity is updated and deleted.
...
- [ ] Unit tests cover `UPDATE` and `DELETE` triggers independently and combined.

---

### US 1.3 — Cancel Running Workflows Declaratively
...
- [ ] Unit tests cover cancellation success, no-op (no execution found), and n8n API error scenarios.

---

### US 1.4 — Map Entity Attributes to Workflow Inputs

**As a** CAP developer,
**I want to** use the `inputs` array in the annotation to map my entity attributes and associations to the JSON payload expected by the n8n workflow
**So that** the workflow receives the correct data from my CAP entity.
...
- [ ] Unit tests validate scalar mapping, association expansion, and missing-element error cases.

---

### US 1.5 — Conditional Workflow Triggers

**As a** CAP developer,
**I want to** use the `if:` annotation expression to conditionally trigger n8n workflows only when specific data conditions are met
**So that** workflows are not triggered unnecessarily for irrelevant data changes.
...
- [ ] Unit tests cover true/false evaluation, complex expressions (e.g., `status = 'approved' AND amount > 1000`), and invalid expression errors.
...
### US 2.1 — Programmatic Workflow Triggering

**As a** CAP developer,
**I want to** connect to a generic `N8nWorkflowService`
**So that** I can programmatically trigger workflows from my custom Node.js event handlers.
...
- [ ] Unit tests cover successful triggering, authentication, and error propagation.

---

### US 2.2 — Cancel Workflows Programmatically
...
- [ ] Unit tests cover successful cancellation, already-completed execution, and API error cases.

---

### US 2.3 — Query Running Executions

**As a** CAP developer,
**I want to** query running n8n executions using tags and execution IDs (representing business keys) via the programmatic API
...
- [ ] Unit tests cover tag-based filtering, ID-based lookup, empty results, and pagination.

---

### US 2.4 — Local Mock Implementation

**As a** CAP developer,
**I want to** a local mock implementation of n8n execution
...
- [ ] Unit tests verify that the mock correctly simulates start, cancel, and query operations.

---

### US 2.5 — Configuration Profiles

**As a** CAP developer,
**I want to** use configuration profiles (e.g., `[development]`, `[production]`) in `package.json` to seamlessly switch between the local mock service and a real n8n instance
**So that** environment-specific behavior is managed through standard CAP configuration.
...
**So that** offline development is possible.
...
- [ ] Unit tests cover import of valid workflow JSON, generation of correct CDS types, and error handling for malformed JSON.
...
- [ ] Unit tests cover remote fetch, CDS generation, and error scenarios.
...
**As a** CAP developer,
**I want to** the `cds build` process to validate my `@n8n.workflow.start` inputs against the imported CDS service, throwing errors if required inputs are missing or typed incorrectly
...
- [ ] Unit tests cover missing inputs, type mismatches, extra inputs, and missing import scenarios.
...
- [ ] Unit tests validate credential configuration and test connectivity logic.
...
**I want to** use the CAP node in Query mode to fetch a collection of entities from a CAP OData service with optional filtering, sorting, and pagination
**So that** I can retrieve a relevant subset of records to drive decisions or iteration in my workflow.
...
- [ ] Unit tests cover query with filters, sorting, pagination, and error responses.
...
**As an** n8n workflow designer,
**I want to** use the CAP node in Read mode to fetch a specific entity by its primary key
**So that** I can look up individual records to drive downstream decisions in my workflow.
...
- [ ] Unit tests cover successful read, not-found, and composite key scenarios.
...
- [ ] Unit tests cover successful creation, validation errors, and server errors.
...
- [ ] Unit tests cover successful update, partial update, not-found, and server error scenarios.
...
- [ ] Unit tests cover successful deletion, not-found, and server error scenarios.
...
**As an** n8n user,
**I want to** the Action Node to dynamically fetch the `$metadata` of the CAP service to populate a dropdown of available Entity Sets in the n8n UI
**So that** I don't have to manually type entity set names and can discover the service schema interactively.
...
- [ ] Unit tests cover metadata parsing, dropdown population, and error handling.
...
**As an** n8n user,
**I want to** invoke CAP actions/functions
**So that** workflows can execute business logic exposed by the CAP service.
...
- [ ] Unit tests cover bound actions, unbound actions, function imports, and error scenarios.
...
**As an** n8n user,
**I want to** an SAP CAP Trigger Node that periodically polls a CAP OData endpoint to detect new or changed records, triggering my workflow automatically
...
- [ ] Unit tests cover polling logic, deduplication, first-run behavior, and error handling.
...
**So that** cloud deployments are supported.
...
**As a** developer,
**I want to** hybrid local/cloud testing
